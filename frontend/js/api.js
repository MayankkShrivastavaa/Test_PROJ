...
// the session cookie ("sid") - without this, the browser won't send or
// accept cookies on a cross-origin fetch (frontend on :5500, backend on
// :5000 during local dev). The earlier registration/OTP/MFA calls never
// needed this, since none of them used cookies.
...
async function loginUser(emailOrUsername, password) {
  try {
    ...
    if (!response.ok) {
      return { ok: false, data, error: data.msg || "Login failed" };
    }
...
async function fetchMe() {
  try {
    ...
    if (!response.ok) {
      return { ok: false, data, error: data.msg || "Not authenticated" };
    }
...
async function logoutUser() {
  try {
    ...
    const data = await response.json();
    return { ok: response.ok, data, error: response.ok ? null : data.msg };
  } catch (networkError) {
    console.log(networkError);
    return {
      ok: false,
